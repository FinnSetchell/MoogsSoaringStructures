package com.finndog.mss.misc.pooladditions;

import com.finndog.mss.MSSCommon;
import com.finndog.mss.events.lifecycle.ServerGoingToStartEvent;
import com.finndog.mss.mixins.structures.ListPoolElementAccessor;
...
import com.finndog.mss.mixins.structures.StructureTemplateManagerAccessor;
...
public final class PoolAdditionMerger {
    ...
    public static void mergeAdditionPools(final ServerGoingToStartEvent event) {
        ResourceManager resourceManager = ((StructureTemplateManagerAccessor) event.getServer().getStructureManager()).mss_getResourceManager();
            ...
            for (JsonElement jsonElement : entry.getValue()) {
                try {
                    ...
                    MSSCommon.LOGGER.error("""
                            ...
                            Moog's Soaring Structures: Pool Addition json failed to be parsed.
    ...
    private static void mergeIntoExistingPool(AdditionalStructureTemplatePool feedingPool, StructureTemplatePool gluttonyPool, StructureTemplateManager structureTemplateManager) {
        ...
        ObjectArrayList<StructurePoolElement> elements = new ObjectArrayList<>(((StructurePoolAccessor) gluttonyPool).mss_getTemplates());
        List<Pair<StructurePoolElement, Integer>> elementCounts = new ArrayList<>(((StructurePoolAccessor) gluttonyPool).mss_getRawTemplates());

        elements.addAll(((StructurePoolAccessor) feedingPool).mss_getTemplates());
        elementCounts.addAll(((StructurePoolAccessor) feedingPool).mss_getRawTemplates());
        ...
        for(StructurePoolElement element : elements) {
            if(element instanceof SinglePoolElement singlePoolElement) {
                Optional<ResourceLocation> pieceRL = ((SinglePoolElementAccessor)singlePoolElement).mss_getTemplate().left();
                ...
                for(StructurePoolElement listElement : ((ListPoolElementAccessor)listPoolElement).mss_getElements()) {
        ...
        ((StructurePoolAccessor) gluttonyPool).mss_setTemplates(elements);
        ((StructurePoolAccessor) gluttonyPool).mss_setRawTemplates(elementCounts);
    }
    ...
    private static void checkIfPieceExists(AdditionalStructureTemplatePool feedingPool, StructureTemplateManager structureTemplateManager, ResourceLocation pieceRL) {
        ...
        try {
            ...
            if (inputstream.available() == 0 || inputstream.read(new byte[1]) == -1) {
                MSSCommon.LOGGER.error("(Moog's Soaring Structures POOL MERGER) Found an entry in {} that points to the non-existent nbt file called {}", feedingPool.getName(), pieceRL);
            }
            inputstream.close();
        }
     ...
     */
    private static void logBadData(ResourceLocation poolPath, String msssageString) {
        MSSCommon.LOGGER.error("(Moog's Soaring Structures POOL MERGER) Failed to parse {} additions file. Error is: {}", poolPath, msssageString);
    }
