...
import com.finndog.mss.mixins.structures.TemplateAccessor;
...
public class MirroringSingleJigsawPiece extends SinglePoolElement {
    ...
    public MirroringSingleJigsawPiece(SinglePoolElement singleJigsawPiece, Mirror mirror, Optional<LiquidSettings> liquidSettings) {
        this(((SinglePoolElementAccessor)singleJigsawPiece).mss_getTemplate(), ((SinglePoolElementAccessor)singleJigsawPiece).mss_getProcessors(), singleJigsawPiece.getProjection(), mirror, liquidSettings);
    }
    ...
    private ObjectArrayList<StructureTemplate.JigsawBlockInfo> getJigsaws(StructureTemplate template, BlockPos blockPos, StructurePlaceSettings structurePlaceSettings) {
        if (((TemplateAccessor)template).mss_getPalettes().isEmpty()) {
            return new ObjectArrayList<>();
        }
        else {
            List<StructureTemplate.JigsawBlockInfo> list = structurePlaceSettings.getRandomPalette(((TemplateAccessor)template).mss_getPalettes(), blockPos).jigsaws();
    ...
    public StructurePoolElementType<?> getType() {
        return MSSStructurePieces.MIRROR_SINGLE.get();
    }
