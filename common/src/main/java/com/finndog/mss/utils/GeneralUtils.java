...
import com.finndog.mss.mixins.resources.NamespaceResourceManagerAccessor;
import com.finndog.mss.mixins.resources.ReloadableResourceManagerImplAccessor;
...
public final class GeneralUtils {
    ...
    public static List<InputStream> getAllFileStreams(ResourceManager resourceManager, ResourceLocation fileID) throws IOException {
        ...
        FallbackResourceManager namespaceResourceManager = ((ReloadableResourceManagerImplAccessor) resourceManager).mss_getNamespacedManagers().get(fileID.getNamespace());
        List<FallbackResourceManager.PackEntry> allResourcePacks = ((NamespaceResourceManagerAccessor) namespaceResourceManager).mss_getFallbacks();
    ...
    public static Map<ResourceLocation, List<JsonElement>> getAllDatapacksJSONElement(ResourceManager resourceManager, Gson gson, String dataType, int fileSuffixLength) {
        ...
        for (ResourceLocation fileIDWithExtension : resourceManager.listResources(dataType, (fileString) -> fileString.toString().endsWith(".json")).keySet()) {
            ...
            try {
                    ...
                    try (Reader bufferedReader = new BufferedReader(new InputStreamReader(fileStream, StandardCharsets.UTF_8))) {
                        ...
                        if (countsJSONElement != null) {
                            ...
                            MSSCommon.LOGGER.error(
                                    "(Moog's Soaring Structures {} MERGER) Couldn't load data file {} from {} as it's null or empty",
                                    dataType,
                                    fileID,
                                    fileIDWithExtension);
                        }
                ...
                MSSCommon.LOGGER.error(
                        "(Moog's Soaring Structures {} MERGER) Couldn't parse data file {} from {}",
                        dataType,
                        fileID,
                        fileIDWithExtension,
                        exception);
